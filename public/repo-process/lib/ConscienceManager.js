...
async function isBehindRemote(repoID, folderPath) {
    ...
    try {
        ...
        let obj = await git.readObject({dir: folderPath, oid: masterHash})
        if (obj.type === 'commit') {
            unlock(folderPath)
            return false
        }
...
async function getFiles(folderPath) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        klaw(folderPath, {
            ...
            // Exclude directories
            if (!item.stats.isDirectory()) { next(null, item) }
            else { next() }
...
async function getFilesFromTree(folderPath, oid, subfolder) {
    let tree
    try {
        tree = await git.readObject({dir:folderPath, oid: oid})
    } catch (err) {
        console.log('ERROR running ConscienceManager.getFilesFromTree ~>', err)
        return []
    }

    let subTrees = tree.object.entries.filter(f => f.type === 'tree')
...
async function getTimeline(folderPath) {
    ...
    for (let i = 0; i < commits.length; i++) {
        ...
        let files = filesByCommit[i].filter(f => {
            ...
            let prev = filesByCommit[i+1].find(f2 => f2.path === f.path)
